...
namespace T39_03_01_DataAccessLibrary
{
	public class MySqlCrud
	{
		...
		public FullContactModel? GetFullContactById(int id)
		{
			...
			{
				BasicInfo = db
					.LoadData<BasicContactModel, dynamic>(sql, new { Id = id }, _connectionString)
					.FirstOrDefault()
			};
			...
			sql = @"select e.*
					...
					inner join ContactEmailAddresses ce on ce.EmailAddressId = e.Id
					where ce.ContactId = @Id";
			output.EmailAddresses = db
				.LoadData<EmailAddressModel, dynamic>(sql, new { Id = id }, _connectionString);

			sql = @"select p.*
					from PhoneNumbers p
					inner join ContactPhoneNumbers cp on cp.PhoneNumberId = p.Id
					where cp.ContactId = @Id";
			output.PhoneNumbers = db
				.LoadData<PhoneNumberModel, dynamic>(sql, new { Id = id }, _connectionString);
			...
			foreach (var phoneNumber in contact.PhoneNumbers!)
			{
				if (phoneNumber.Id == 0)
				{
					...
					phoneNumber.Id = db.LoadData<IdLookupModel, dynamic>(sql,
						new { phoneNumber.PhoneNumber }, _connectionString).First().Id;
				}
			...
			foreach (var email in contact.EmailAddresses!)
			{
				if (email.Id == 0)
				{
					...
					email.Id = db.LoadData<IdLookupModel, dynamic>(sql,
						new { email.EmailAddress }, _connectionString).First().Id;
				}

				sql = "insert into ContactEmailAddresses (ContactId, EmailAddressId) values (@ContactId, @EmailAddressId);";
				db.SaveData(sql,
		...
		public void RemovePhoneNumberFromContact(int contactId, int phoneNumberId)
		{
			...
			var links = db.LoadData<ContactPhoneNumberModel, dynamic>(sql,
				new { PhoneNumberId = phoneNumberId, }, _connectionString);
